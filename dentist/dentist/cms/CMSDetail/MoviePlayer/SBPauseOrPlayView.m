...
- (void)drawRect:(CGRect)rect {
    if (!self.imageBtn) {
        ...
        [self.fastGoBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.mas_equalTo(self).offset(self.frame.size.width/2 + 80);
            make.right.mas_equalTo(self).offset(-self.frame.size.width/4 + 40);
            make.bottom.mas_equalTo(self).offset(-self.frame.size.height/2 + 40);
            make.size.width.height.mas_equalTo(80);
        }];
        
        self.fastBackBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        //    self.fastBackBtn.backgroundColor = [UIColor redColor];
        ...
        [self.fastBackBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.mas_equalTo(self).offset(self.frame.size.width/4 - 40);
            make.right.mas_equalTo(self).offset(-self.frame.size.width/2 - 80);
            make.bottom.mas_equalTo(self).offset(-self.frame.size.height/2 + 40);
            make.size.width.height.mas_equalTo(80);
        }];
    }
