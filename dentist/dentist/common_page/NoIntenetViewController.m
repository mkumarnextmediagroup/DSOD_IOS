...
- (void)viewDidLoad {
	...
	UIView *btnPanel = self.view.addView;
	UIButton *retryBtn = btnPanel.addButton;
	[retryBtn title:localStr(@"Retry")];
	[retryBtn stylePrimary];
	[[[[[retryBtn.layoutMaker widthEq:98] heightEq:BTN_HEIGHT] leftParent:0] centerYParent:0] install];

	UIButton *needBtn = btnPanel.addButton;
	[needBtn title:localStr(@"Need help?")];
	[needBtn styleWhite];
	[[[[[[needBtn layoutMaker] toRightOf:retryBtn offset:10] rightParent:0] centerYParent:0] heightEq:BTN_HEIGHT] install];

	StackLayout *sl = [StackLayout new];
	[sl push:btnPanel height:BTN_HEIGHT marginBottom:65];
