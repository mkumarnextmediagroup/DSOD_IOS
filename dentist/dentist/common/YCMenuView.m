...
+ (instancetype)menuWithArray:(NSArray<IdName *> *)actions width:(CGFloat)width atPoint:(CGPoint)point{
    NSAssert(width>0.0f, @"width要大于0");
    YCMenuView *menu = [[YCMenuView alloc] initWithActions:actions width:width atPoint:point];
    return menu;
}
...
+ (instancetype)menuWithArray:(NSArray<IdName *> *)actions width:(CGFloat)width relyonView:(id)view
{
    NSAssert(width>0.0f, @"width要大于0");
    NSAssert([view isKindOfClass:[UIView class]]||[view isKindOfClass:[UIBarButtonItem class]], @"relyonView必须是UIView或UIBarButtonItem");
    YCMenuView *menu = [[YCMenuView alloc] initWithActions:actions width:width relyonView:view];
...
- (instancetype)initWithActions:(NSArray *)actions width:(CGFloat)width atPoint:(CGPoint)point{
    if (self = [super init]) {
...
- (instancetype)initWithActions:(NSArray *)actions width:(CGFloat)width relyonView:(id)view{
    if (self = [super init]) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (_actions.count>indexPath.row) {
        id model=_actions[indexPath.row];
        if ([model isKindOfClass:[YCMenuAction class]]) {
        ...
        }else if([model isKindOfClass:[IdName class]]){
            IdName *idmodel=(IdName *)model;
            cell.textLabel.text=idmodel.name;
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (_actions.count>indexPath.row) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(didYCMenuSelectItemAtIndex:)]) {
            [self.delegate didYCMenuSelectItemAtIndex:indexPath.row];
        }
    }
...
- (UIView *)bgView{
    if (!_bgView) {
        ...
        _bgView.backgroundColor = [UIColor clearColor];//[[UIColor blackColor] colorWithAlphaComponent:0.1];
        _bgView.alpha = 0.0f;
